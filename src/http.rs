...
impl Http {
  ...
  async fn get_last_pokemon(&self) -> anyhow::Result<i64> {
    ...
    let res = self.client.post(Self::POKEAPI_GRAPHQL_API).body(serde_json::to_vec(&body)?).send().await?;
    let query: GraphQLQueryResponse<PokemonQueryResponse<LastPokemonQueryResponseFields>> = res.json().await?;
  ...
  pub async fn get_pokemon(&self, graphql_query: impl AsRef<str>) -> anyhow::Result<Pokemon> {
    ...
    let res = self.client.post(Self::POKEAPI_GRAPHQL_API).body(serde_json::to_vec(&body)?).send().await?;
    let query: GraphQLQueryResponse<PokemonQueryResponse<PokemonSpecyAndSpritesQueryResponseFields>> = res.json().await?;
    Ok(Pokemon {
