...
use crate::{pokeapi_db::prelude::*, Pokemon};
...
impl Db {
  ...
  pub async fn new(db_path: &Option<impl AsRef<Path>>) -> anyhow::Result<Self> {
...

    {
        ...
        const DB_BYTES: &[u8] = include_bytes!(include_str!(concat!(env!("OUT_DIR"), "/embed_db_path")));
        conn.deserialize(
            None,
            SqliteOwnedBuf::try_from(DB_BYTES)?,
            true,
        )
        .await?;
    }
  ...
  pub async fn new(db_path: &Option<impl AsRef<Path>>) -> anyhow::Result<Self> {
    let db = Database::connect(path_to_connect_options(db_path.as_ref().ok_or_else(|| anyhow::anyhow!("Database not provided!"))?.as_ref())).await?;
    Ok(Self { db })
  }
  ...
  pub async fn get_pokemon(
    ...
    Ok(Pokemon { name: species_name.name, sprite_url: sprites.sprites })
  }
